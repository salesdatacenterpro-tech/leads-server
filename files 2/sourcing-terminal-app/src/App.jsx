...
import { Plus, Trash2, TrendingUp, TrendingDown, Minus, Package, Download, Search, Zap, AlertCircle, CheckCircle2, X, RefreshCw, Filter, BarChart3, Eye, Globe, ExternalLink, Key } from 'lucide-react';
...
Les scores sont sur 100. La marge B2C suppose revente prix marche moyen. La marge B2B suppose revente a -15 a -25% du prix marche. Sois realiste et precis, si le produit est inconnu ou la reference trop vague, mets confidence "Basse" et explique dans keyInsights.`;
  ...
  // Recupere la cle API depuis le localStorage
  const apiKey = localStorage.getItem('anthropic-api-key');
  if (!apiKey) {
    throw new Error('Cle API Anthropic manquante. Clique sur le bouton "Cle API" en haut a droite pour la configurer.');
  }
    ...
    <div style={styles.app}>
      ...
      <header style={styles.header}>
        ...
        <div style={styles.headerRight}>
          <button style={styles.apiKeyBtn} onClick={() => setShowApiKeyModal(true)} title="Configurer la cle API Anthropic">
            <Key size={11} />
            {localStorage.getItem('anthropic-api-key') ? 'API OK' : 'CLÉ API'}
          </button>
...
function ApiKeyModal({ onClose }) {
  ...
  const hasExisting = !!localStorage.getItem('anthropic-api-key');

  const save = () => {
    const trimmed = key.trim();
    if (!trimmed) return alert('Cle vide');
    if (!trimmed.startsWith('sk-ant-')) {
      if (!confirm('La cle ne commence pas par "sk-ant-". Enregistrer quand meme ?')) return;
    }
  ...
  const clear = () => {
    if (!confirm('Supprimer la cle API ? Tu devras la ressaisir pour utiliser l\'analyse IA.')) return;
    ...
    <div style={styles.modalOverlay} onClick={onClose}>
      <div style={{ ...styles.modal, maxWidth: 520 }} onClick={e => e.stopPropagation()}>
        <div style={styles.modalHeader}>
          <h2 style={styles.modalTitle}>Clé API Anthropic</h2>
          <button style={styles.iconBtn} onClick={onClose}><X size={16} /></button>
        </div>
        <div style={{ padding: 24 }}>
          <p style={{ fontSize: 13, color: 'var(--muted-2)', lineHeight: 1.6, margin: '0 0 16px' }}>
            L'analyse IA utilise l'API Claude d'Anthropic. Ta clé est stockée uniquement dans ton navigateur (localStorage) et n'est jamais envoyée ailleurs que sur les serveurs d'Anthropic.
          </p>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <input
              ...
              autoFocus
            />
            <button style={styles.btnGhost} onClick={() => setShowKey(s => !s)} title={showKey ? 'Masquer' : 'Afficher'}>
              <Eye size={14} />
            </button>
          </div>
          <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 10, lineHeight: 1.5 }}>
            Pas encore de clé ? Crée-la sur{' '}
            <a href="https://console.anthropic.com/settings/keys" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--accent)' }}>
              console.anthropic.com/settings/keys
            </a>
            . Coût typique : ~0.002 à 0.01 € par analyse d'offre (modèle Sonnet).
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 24 }}>
            {hasExisting ? (
              <button type="button" style={{ ...styles.btnGhost, color: 'var(--alert)', borderColor: 'var(--alert)' }} onClick={clear}>
                <Trash2 size={12} /> Supprimer
              </button>
            ) : <span />}
            <div style={{ display: 'flex', gap: 8 }}>
              <button type="button" style={styles.btnGhost} onClick={onClose}>Annuler</button>
              <button type="button" style={styles.btnPrimary} onClick={save}>Enregistrer</button>
            </div>
          </div>
        </div>
      </div>
...
const styles = {
  ...
  apiKeyBtn: { display: 'inline-flex', alignItems: 'center', gap: 5, padding: '5px 10px', background: 'transparent', color: 'var(--muted-2)', border: '1px solid var(--border)', borderRadius: 3, fontSize: 10, fontWeight: 700, letterSpacing: 1, cursor: 'pointer', fontFamily: 'var(--font-mono)' },
